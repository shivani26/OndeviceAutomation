...
public class NotFoundMismatchFingerprint {
    public static void main(String[] args) throws Exception{
         ...
         */
        JSONArray songsMetaData = convertToJSONArray(System.getProperty("user.dir") + "/testdataFG/fgdata.json");
        String matchResult = readAll(new FileReader(System.getProperty("user.dir") + "/testdataFG/fgresultafter.json"));
        ...
        File file = new File("testdataFG/FPmismatchnotfound.json"); //Your file
        ...
        for(int i = 0; i < songsMetaData.length() && i < jsons.length; i++){
            if(!jsons[i].trim().equalsIgnoreCase("No Match Found")){
                ...
                {
                    System.out.println((i+1) + "  FING: No Match Found"  + ",  OndeviceTitle :: "+ getCleanedString(get(songsMetaData.getJSONObject(i), "title")));
                ...
                else
                {
                    ...
                    String onlineId = getCleanedString(get(json, "contentId"));
                    //String onlineId = getCleanedString(get(songsMetaData.getJSONObject(i), "contentId"));
                    ...
                    {
                        System.out.println((i+1) + "  FING: Title/Album Mismatch :: "+ ",  Online Id :: " +onlineId + ",  OndeviceTitle :: "  + offlineTitle + ", OnlineTitle :: " + onlineTitle + ", OndeviceAlbum :: " + offlineAlbum + ", OnlineAlbum :: " + onlineAlbum);
                        continue;
                    }
                    else
                    {
                        System.out.println((i+1) + "  FING: Content Found :: "+ ",  OndeviceTitle :: "  + offlineTitle + ", OnlineTitle :: " + onlineTitle + ", OndeviceAlbum :: " + offlineAlbum + ", OnlineAlbum :: " + onlineAlbum);
                        continue;
                    }
                }
            }
